<script>
    ...
    let imagePath = null;
    let videoPath = null;
    const serverUrl = 'http://191.252.195.85:5001/api/';
        ...
        <form on:submit={handleSubmit} class="flex flex-col gap-4">
            ...
            <video id="videoPreview" width="320" height="240" controls>
                {#if exercise.videoPath}
                    <source src={`${serverUrl}http://191.252.195.85:5001/api/Files/${exercise.videoPath}`} />
                {/if}
                <track kind="captions" src="path/to/captions.vtt" srclang="en" label="English">
                Seu navegador não suporta o elemento de vídeo.
            </video>
            <p>{serverUrl}http://191.252.195.85:5001/api/Files/{exercise.videoPath}</p>
