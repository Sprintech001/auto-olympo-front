<script>
    ...
    async function deleteUserExercise(userId, exerciseId) {
        ...
        try {
            const response = await fetch(`http://localhost:5001/api/userexercise?userId=${userId}&exerciseId=${exerciseId}`, {
                method: 'DELETE',
            });
...
<div class="w-full flex flex-col gap-4">
    {#if exerciseDetails}
        <a
            ...
            style={`background-image: url('http://localhost:5001/api/Files/${exerciseDetails.imagePath}')`}
        >
