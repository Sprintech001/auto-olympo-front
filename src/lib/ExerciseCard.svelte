<script>
    ...
    async function getExerciseById(exerciseId) {
        try {
            const response = await fetch(`/api//api/exercise/${exerciseId}`);
    ...
    async function deleteUserExercise(userId, exerciseId) {
        ...
        try {
            const response = await fetch(`/api//api/userexercise?userId=${userId}&exerciseId=${exerciseId}`, {
                method: 'DELETE',
            });
...
<div class="w-full flex flex-col gap-4">
    {#if exerciseDetails}
        <a
            ...
            style={`background-image: url('/api//api/Files/${exerciseDetails.imagePath}')`}
        >
